...
input_folder = leia_file_path + 'jpc'
...
output_base_path = leia_file_path + f'/camera/processing_n2/data/files_mnsb_data/jpc/'
masked_folder = output_base_path + 'mnsb_files/'
numpy_folder = output_base_path + 'numpy_files/'
csv_output_folder = leia_file_path + '/camera/processing_n2/data/csvs_mnsb_data/'
...
# Load CSV file
csv_file_path = os.path.join(data_models_path, f'jpc_data.csv')
...
for index, row in df.iterrows():
    image_filename = row['photo_id']  # e.g., '20250208_000126_foto.jpg'
    ...
    filename = os.path.splitext(matched_image)[0]  # '20250208_000126_foto'
    ...
    r_mean = np.mean(r_norm)
    g_mean = np.mean(g_norm)
    b_mean = np.mean(b_norm)

    # Store the mean values in the DataFrame
    df.at[index, 'r_mean'] = round(r_mean, 5)  # Round for better readability
...
# Save updated CSV
df.to_csv(os.path.join(csv_output_folder, f'{date_formatted_empty}_processed.csv'), index=False)

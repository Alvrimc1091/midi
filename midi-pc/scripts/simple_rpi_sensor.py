...
# Path to meassures
directory_path = "/home/pi/demo/data/training3/fosfato"
...
def guardar_datos(datos, foto_id, hora_santiago):
    ...
    with open(f"/home/pi/demo/data/training3/fosfato/{hora_santiago.strftime('%Y%m%d_%H%M%S')}_data.csv", mode='a', newline='') as archivo_csv:
...
def tomar_foto():
    try:
        picam2.start()
        hora_santiago = datetime.datetime.now(zona_santiago)
        nombre_foto = f"/home/pi/demo/data/training3/fosfato/{hora_santiago.strftime('%Y%m%d_%H%M%S')}_foto.jpg"
